...
class StarXpand {
  ...
  static Future<String> monitor(StarXpandPrinter printer,
    ...
    var guid = _addCallbackHandler(
        StarXpandCallbackHandler<StarXpandPrinterStatusPayload>(
            (payload) => callback.call(payload),
            (type, data) => StarXpandPrinterStatusPayload(
                type, Map<String, dynamic>.from(data))));

    await _channel.invokeMethod(
        'monitor', {"printer": printer.toMap(), "callback": guid});
  ...
  static Future<bool> closePrinterConnection(StarXpandPrinter printer) async {
    return await _channel
        .invokeMethod('closeConnection', {"printer": printer.toMap()});
  }

  static Future<bool> printRawBytes(
      StarXpandPrinter printer, Uint8List bytes) async {
    return await _channel.invokeMethod(
        'printRawBytes', {'printer': printer.toMap(), 'bytes': bytes});
  }
